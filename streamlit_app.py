...
_USER_COMPANY = {"KUB": "Kubler", "POS": "Posital", "AQB": "AQB Solutions"}
# Login prefix → default target manufacturer (who the user wants to find replacements FOR)
_PREFIX_TARGET = {"KUB": "Kubler", "POS": "Posital", "AQB": "Kubler"}

if "logged_in" not in st.session_state:
    st.session_state["logged_in"] = False
    st.session_state["user_id"]   = ""
    st.session_state["company"]   = ""
    st.session_state["target_mfr"] = "Kubler"
...
for k, v in [("pn_value",""),("results",None),("status",None),("exps",None),
              ...
              ("target_mfr", _PREFIX_TARGET.get(_USER_ID[:3], "Kubler"))]:
...
def _fmt_ppr(r):
    ...
    if str(r.get("is_programmable","")).lower() in ("true","1"):
        ...
        if lo and hi: return f"{int(float(lo)):,}–{int(float(hi)):,} PPR"
        if hi: return f"1–{int(float(hi)):,} PPR"
        return "Programmable"
...
with st.sidebar:
    ...
    if _IS_ADMIN:
        ...
        # Derive target from session state (set at login by prefix)
        _TGT_MFR    = st.session_state.get("target_mfr", "Kubler")
        _SRC_FILTER = None   # auto-detect from part number
        # Target badge (dynamic — works for Kubler and Posital logins)
        st.markdown(f"""
