...
# ── Safe cast helpers (required because DuckDB all_varchar=True returns all ──
# columns as strings, so int('1.0') fails; use int(float(v)) pattern instead) ─

def safe_int(v):
    """Convert v to int via float, return None on failure."""
    if v is None:
        return None
    try:
        f = float(v)
        import math as _math
        return None if _math.isnan(f) else int(f)
...
COLUMN_MAP = {
    ...
    "order_pattern":"Order Pattern","product_url":"Product URL","resolution_ppr":"Resolution (PPR)",
